...
### 3.1.2 빅데이터 분석 방법론 – 분석기획 단계

| 단계                          | 설명                                                         | 주요 산출물                                                  |
| ----------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **프로젝트 범위 설정**        | 분석 프로젝트의 목적과 범위를 명확히 규정하고, <br />해결해야 할 문제와 기대 효과를 정의하는 단계. | - 프로젝트 범위 정의서 <br />- 문제 인식 및 목표 문서 <br />- 기대 효과 보고서 |
| **데이터 분석 프로젝트 정의** | 프로젝트의 구체적인 분석 목표, 활용 데이터, <br />적용 기법 등을 정의하여 프로젝트의 방향성을 확립하는 단계. | - 데이터 분석 프로젝트 정의서 <br />- 분석 목표 및 가설 설정 문서 <br />- 활용 데이터 목록 |
| **프로젝트 수행계획 수립**    | 프로젝트를 수행하기 위한 일정, 자원, 역할, 방법론 등을 계획하는 단계. | - 프로젝트 수행계획서 <br />- 일정표(Gantt Chart 등) <br />- 역할 및 책임 분담표 |
| **데이터 분석 위험 식별**     | 프로젝트 수행 과정에서 발생할 수 있는 위험 요소<br />(데이터 품질, 기술적 제약, 인력 부족 등)를 파악하고 대응 방안을 마련하는 단계. | - 위험 식별 보고서 <br />- 위험 관리 계획서 <br />- 대응 전략 문서 |
...
**핵심 답변:**
 SPICE는 *Software Process Improvement and Capability dEtermination*의 약자로, **ISO/IEC 15504 표준**에 기반한 소프트웨어 프로세스 품질 및 능력 평가 모델입니다. 데이터 분석 모델의 품질을 평가할 때도 이 프레임워크를 응용하여 **프로세스 성숙도와 수행 능력**을 체계적으로 진단할 수 있습니다.

------

## 📌 SPICE (ISO/IEC 15504) 개요

- **정식 명칭:** Software Process Improvement and Capability dEtermination
- **국제 표준:** ISO/IEC 15504
- **목적:** 조직의 소프트웨어 및 데이터 분석 프로세스를 평가하고 개선 방향을 제시
- **특징:**
  - 기존 품질 관리 모델(CMM, ISO9000, ISO12207 등)을 통합한 국제 표준
  - 프로세스 차원과 능력 차원이라는 **2차원 평가 모델**을 사용
...
| 차원                                  | 설명                                                     | 세부 내용                                                    |
| ------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------ |
| **프로세스 차원 (Process Dimension)** | 조직이 수행하는 프로세스를 범주별로 정의                 | - 고객/공급자 프로세스 - 엔지니어링 프로세스 - 지원 프로세스 - 관리 프로세스 - 조직 프로세스 |
| **능력 차원 (Capability Dimension)**  | 각 프로세스가 얼마나 성숙하고 체계적으로 수행되는지 평가 | - CL0: 불완전 (Incomplete) - CL1: 수행 (Performed) - CL2: 관리 (Managed) - CL3: 확립 (Established) - CL4: 예측 (Predictable) - CL5: 최적화 (Optimizing) |

------

## 📊 데이터 분석 모델 품질 평가에 SPICE 활용

- **분석 프로세스 정의:** 데이터 수집, 전처리, 모델링, 평가, 배포 등 단계별 프로세스를 SPICE 기준에 맞춰 정의
- **능력 수준 평가:** 각 단계가 단순 수행(CL1)인지, 관리 체계(CL2)까지 갖췄는지, 혹은 최적화(CL5)까지 이뤄졌는지 평가
- **개선 방향 도출:** 데이터 품질 관리, 모델 성능 검증, 위험 관리 등 부족한 부분을 체계적으로 개선

------

## 🚨 유의할 점

- SPICE는 **프로세스 평가 모델**이지, 모델 성능 지표(정확도, RMSE 등)를 직접 측정하는 도구는 아닙니다.
- 따라서 **정량적 성능 평가 지표**와 함께 **프로세스 성숙도 평가**를 병행해야 모델 품질을 종합적으로 판단할 수 있습니다.
...
벤 프라이의 시각화 방법론은 데이터 시각화를 위한 7단계 프로세스를 제시합니다. 
\- **획득(Acquire)**: 데이터를 수집하는 단계입니다. 이는 다양한 소스에서 데이터를 가져오는 것을 의미합니다.
\- **파싱(Parse**): 데이터를 구조화하는 단계입니다. 수집된 데이터를 분석하기 쉬운 형태로 변환합니다.
\- **필터링(Filter)**: 데이터를 정제하고 필요한 부분만 남기는 단계입니다.
\- **마이닝(Mine)**: 데이터를 분석해 패턴이나 유의미한 정보를 추출하는 단계입니다.
\- **표현(Represent)**: 데이터를 시각적으로 표현하는 단계입니다.
\- **정제(Refine)**: 시각적으로 표현된 데이터를 더 명확하고 이해하기 쉽게 다듬는 단계입니다.
\- **상호작용(Interact)**: 사용자가 데이터와 상호작용할 수 있게 하는 단계입니다.
