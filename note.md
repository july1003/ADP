...
**간단 답변:**
 MySQL은 단순한 오픈소스 RDBMS로만 알려져 있지만, **MySQL Cluster(NDB Cluster)**라는 기능을 통해 클러스터링과 분산 저장을 지원합니다. 즉, 여러 노드가 하나의 데이터베이스를 공유하면서 **고가용성, 확장성, 폴트 톨러런스**를 제공할 수 있습니다.

------

### 📊 MySQL 주요 특징 (ADP 시험 대비)

| 구분                           | 특징                                                         |
| ------------------------------ | ------------------------------------------------------------ |
| **기본 성격**                  | 오픈소스 RDBMS, SQL 기반, ACID 트랜잭션 지원                 |
| **스토리지 엔진**              | InnoDB(기본), MyISAM, NDBCluster 등 다양한 엔진 선택 가능    |
| **클러스터링 (MySQL Cluster)** | - Shared-nothing 구조 - 자동 샤딩(auto-sharding) - 다중 마스터 구조 - 노드 장애 시에도 서비스 지속 (폴트 톨러런스) |
| **확장성**                     | 수평적 확장 가능, 노드 추가로 처리량 증가                    |
| **고가용성**                   | 99.999% 가용성 목표, 동기/비동기 복제 지원                   |
| **데이터 저장 방식**           | 메모리 기반 + 디스크 기반 혼합 가능                          |
| **인터페이스**                 | SQL + NoSQL (Java, C++, Memcached, HTTP 등)                  |
| **운영 관리**                  | 온라인 스키마 변경, 온라인 백업, GUI 기반 관리 툴 제공       |
| **응용 분야**                  | 실시간 통신, IoT, 대규모 사용자 서비스, 분산 환경            |
...
- **MySQL도 클러스터링이 가능하다** → NDB Cluster 엔진 사용.
- **Shared-nothing 구조** → 노드 간 공유 디스크 없이 독립적으로 운영.
- **자동 샤딩** → 데이터가 자동으로 여러 노드에 분산 저장.
- **다중 마스터** → 모든 노드가 쓰기 가능, 단일 장애점 없음.
- **고가용성** → 노드 일부 장애에도 서비스 지속.

------

✅ 요약:
 MySQL은 단순히 소규모 웹 서비스용 DB가 아니라, **MySQL Cluster**를 통해 **분산 데이터 저장, 클러스터링, 고가용성**을 지원합니다. ADP 시험에서는 “MySQL도 클러스터링이 가능하다”는 점과 **NDB Cluster 엔진의 특징**을 기억하는 것이 핵심이에요.

---

좋습니다 👍 ADP 2과목 대비용으로 **MySQL vs Oracle RAC vs NoSQL**의 특징을 비교하는 표를 만들어 드릴게요.

------

### 📊 분산 데이터 저장 기술 비교

| 구분                 | **MySQL Cluster (NDB)**                         | **Oracle RAC**                                 | **NoSQL DB (예: MongoDB, Cassandra)**           |
| -------------------- | ----------------------------------------------- | ---------------------------------------------- | ----------------------------------------------- |
| **구조**             | Shared-nothing 구조, 자동 샤딩                  | Shared-disk 구조, 모든 노드가 동일 DB 접근     | Key-Value, Document, Column 기반 등 다양한 구조 |
| **클러스터링**       | 지원 (NDB Cluster 엔진)                         | 지원 (Real Application Clusters)               | 기본적으로 분산 구조, 클러스터링 내장           |
| **확장성**           | 수평적 확장 가능, 노드 추가로 처리량 증가       | 수평 확장 제한적, 주로 고가용성 중심           | 수평 확장에 최적화, 대규모 데이터 처리          |
| **고가용성**         | 노드 장애 시 다른 노드가 서비스 지속            | 노드 장애 시 다른 노드가 이어받음              | 데이터 복제와 파티셔닝으로 고가용성 확보        |
| **데이터 저장 방식** | 메모리 기반 + 디스크 기반 혼합                  | 디스크 기반, 고성능 스토리지 필요              | 대부분 디스크 기반, 일부 인메모리 옵션          |
| **트랜잭션**         | ACID 지원 (InnoDB, NDB)                         | 강력한 ACID 지원                               | 일부는 ACID 미지원, BASE 모델 채택              |
| **응용 분야**        | 웹 서비스, 통신, IoT, 중간 규모 시스템          | 금융, 대기업 핵심 시스템, 미션 크리티컬 서비스 | 빅데이터, 로그 분석, 실시간 추천, IoT           |
| **장점**             | 오픈소스, 클러스터링 가능, 유연한 스토리지 엔진 | 강력한 안정성, 성능, 폴트 톨러런스             | 대규모 데이터 처리, 유연한 스키마               |
| **단점**             | 대규모 환경에서는 관리 복잡                     | 비용 매우 높음, 관리 복잡                      | 트랜잭션 일관성 부족 가능, 학습 곡선            |

------

✅ **시험 포인트 정리**

- **MySQL도 클러스터링 가능하다** → NDB Cluster 엔진.
- **Oracle RAC** → Shared-disk 구조, 폴트 톨러런스와 고가용성에 강점.
- **NoSQL** → 수평 확장성과 유연한 스키마, 빅데이터 처리에 최적.

------

이제 ADP 시험에서 “분산 데이터 저장 기술” 문제 나오면, **MySQL도 클러스터링 지원한다**는 점을 확실히 기억하면 됩니다.
